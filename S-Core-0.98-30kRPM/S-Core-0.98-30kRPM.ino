...
#define QUICKREADTRIGGER() ((PIND & /* trig_a_pin */ 0b01000000) && !(PIND & /* trig_b_pin */ 0b10000000))
...
unsigned long setpointRPM = 15000;
...
unsigned long runSpeedMax = 75000/(500 /* Rounds Per minute */); //Bolt speed absolute maximum - limit precisely
...
bool decelerateBoltToSwitch() {
  ...
  stepsToGo = 150;
  stepdelay = currSpeed;
  while ((stepsToGo > 0) && (PINB & 0b00010000)) {
    commutate(stepdelay);
...
void loop() {
  if (firstRun) {
    ...
    if (digitalRead(trig_a_pin) && !digitalRead(trig_b_pin)) {
      ...
      while (digitalRead(trig_a_pin) && !digitalRead(trig_b_pin)) {
